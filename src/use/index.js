...
export const user_real_balance = reactive(ref(0))
...
export const keys_list = reactive(ref({
    ...
    keyword: {
        ...
        // prompt: 'Такие как: профессия, город, ...',
...
export const uploadFile = (event, list, modal) => {
    ...
    reader.onload = () => {
        if (reader.result) list = reader.result.trim().split(',').map(item => item.trim());
        modal = true
    };
...
export function refresh() {
    ...
    //prohibited_model.value = false;
    //keywords_model.value = false;
